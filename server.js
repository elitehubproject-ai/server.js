...
function getProfile(userId) {
    ...
    return {
        ...
        avatar: normalizeText(raw.avatar || fallbackUser.avatar || '', 2000000),
        cover: normalizeText(raw.cover || '', 2000000),
...
function ensureMessengerUser(userId, payload = {}) {
    ...
    if (!messengerStore.users[uid]) {
        messengerStore.users[uid] = {
            ...
            avatar: normalizeText(payload.avatar || '', 2000000),
        ...
        const nextAvatar = normalizeText(payload.avatar || user.avatar || '', 2000000);
    ...
    messengerStore.profiles[uid] = {
        ...
        avatar: normalizeText(existingProfile.avatar || messengerStore.users[uid].avatar || '', 2000000),
        cover: normalizeText(existingProfile.cover || '', 2000000),
                ...
                case 'messenger-profile-update':
                    {
                        ...
                        const nextProfile = {
                            ...
                            avatar: normalizeText(data.avatar ?? profile.avatar, 2000000),
                            cover: normalizeText(data.cover ?? profile.cover, 2000000),
                            updatedAt: nowMs()
                        };
